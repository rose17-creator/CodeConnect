...
const ShareChallenge = ({ open, setOpen, challenge }: { open: boolean, setOpen: any, challenge: Challenge }) => {
    ...
    const handleShare = () => {
        if (type == 'friend') {
            ...
            dispatch<any>(shareChallenge(challenge, selectedFriends, setOpen))
        }
        else {
            if (selectedGroups.length == 0) return alert('Please select someone to share post.')
            dispatch<any>(shareChallengeInGroups(challenge, selectedGroups, setOpen))
        }
    }
