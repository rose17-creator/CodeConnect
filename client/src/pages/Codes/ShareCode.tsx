...
const ShareCode = ({ open, setOpen, code }: { open: boolean, setOpen: any, code: Code }) => {
    ...
    const handleShare = () => {
        if (type == 'friend') {
            ...
            dispatch<any>(shareCode(code, selectedFriends, setOpen))
        }
        else {
            if (selectedGroups.length == 0) return alert('Please select someone to share post.')
            dispatch<any>(shareCodeInGroups(code, selectedGroups, setOpen))
        }
    }
