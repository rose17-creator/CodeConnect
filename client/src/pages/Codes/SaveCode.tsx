...
const SaveCode = ({ open, setOpen, code }: { open: boolean, setOpen: any, code: Code }) => {
    ...
    ///////////////////////////////////////////////////// FUNCTIONS ////////////////////////////////////////////////////
    const handleSave = () => {
        dispatch<any>(saveCodeInCollections(code, selectedCollections, setOpen));
        ...
        >
            ...
            <div className="grid md:grid-cols-2 grid-cols-1 gap-4 my-4 ">
                {userCollections.map((collection, index) => {
                    let disabled = collection.codes.findIndex(c => c._id == code._id) != -1
                    return (
                        <Button
                            key={index}
                            disabled={disabled}
                        ...
                        >
                            ...
                            <span className={`font-medium text-md ${selectedCollections.includes(collection._id) ? 'text-white' : 'text-foreground '}`}>{collection.codes.length} Codes</span>
                        </Button>
                    )
                })}
            </div>
